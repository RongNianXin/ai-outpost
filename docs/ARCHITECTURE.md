...
`config/content-network.json` 是 AI 前哨站网络资源登记表，期刊 JSON 是版本与事实的唯一来源。同步采用事件驱动：`website_publish` 先把期刊切为公开状态，再在同一发布事务内更新官网构建输入和 GitHub README，执行 `content:check:network-sync`，随后才允许提交、推送与部署。任何原子资源校验失败都中止该次发布；失败前生成的 README 会随发布事务回退。

公众号、小红书等外部平台登记为 `authorization_required`。官网发布只生成其同源稿包和待办状态，不取得平台写权；收到针对具体期号、平台和动作的授权后才发布并回读证据。后续新增入口时扩充登记表，不修改散落的硬编码清单。定时任务只用于发现遗漏，不承担正确性的主链路，避免调度随会话或环境消失后造成静默错期。
